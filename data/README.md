...
| **ClinVar** | `clinvar_cftr.tsv` | ~6,100 (drifts weekly) | `benchmark/00_clinvar.ipynb` | `variant_summary.txt.gz`, streamed from `ftp.ncbi.nlm.nih.gov/pub/clinvar/tab_delimited/` and filtered to `GeneSymbol=='CFTR', Assembly=='GRCh38'` | **Release date UNPINNED** — ClinVar updates ~weekly; record the exact retrieval date, since it drives the A1 VUS set |
